...
const sendLocationToBackend = async (latitude, longitude) => {
  try {
    ...
    if (!data) throw new Error("No rider data found");
...
const Pending = () => {
  ...
  const snapPoints = useMemo(() => {
    if (filteredOrders[0]?.orderStatus === "Arrived") {
      return ["90%"]; // Customize these values as needed
    } else {
      return ["40%", "90%"]; // Default snap points
    }
  }, [filteredOrders[0]?.orderStatus]);

  // callbacks
  const handleSheetChanges = useCallback((fromIndex, toIndex) => {
    // console.log("Bottom sheet moved from", fromIndex, "to", toIndex);
  }, []);
  ...
  useEffect(() => {
    TaskManager.defineTask(LOCATION_TASK_NAME, ({ data }) => {
      if (data && data.locations.length > 0) {
        ...
        // Animate to rider's new location
        mapRef.current?.animateToRegion(
          {
            latitude,
            longitude,
            latitudeDelta: 0.002, // Set consistent zoom level
            longitudeDelta: 0.004, // Set consistent zoom level
          },
  ...
  const defaultRegion = {
    latitude: riderCoords.latitude || 5.788121544177521,
    longitude: riderCoords.longitude || 6.108269691467286,
    latitudeDelta: 0.0522,
    longitudeDelta: 0.0421,
  };
  const [loadingMap, setLoadingMap] = useState(true);

  const storeCoords = orders[0]?.storeId?.coords;
  const deliveryCoords = orders[0]?.deliveryAddress;

  useEffect(() => {
    const updateRoute = async () => {
      if (riderCoords && deliveryCoords) {
        const { latitude: riderLat, longitude: riderLng } = riderCoords;
        const { latitude: destinationLat, longitude: destinationLng } =
          deliveryCoords;

        // Fetch directions with updated rider's coordinates
        await fetchDeliveryDirections(
          riderLat,
          riderLng,
          destinationLat,
          destinationLng
        ...
        // Smooth animation to the rider's current location
        mapRef.current?.animateToRegion(
          {
            latitude: riderLat,
            longitude: riderLng,
            latitudeDelta: 0.002, // Consistent zoom
            longitudeDelta: 0.004, // Consistent zoom
          },
          1000 // Duration of the animation (1000ms = 1 second)
        );
      }
    };

    const updateStoreRoute = async () => {
      if (riderCoords && storeCoords) {
        const { latitude: riderLat, longitude: riderLng } = riderCoords;
        const { latitude: destinationLat, longitude: destinationLng } =
          storeCoords;

        // Fetch directions with updated rider's coordinates
        await fetchStoreDirections(
          riderLat,
          riderLng,
          destinationLat,
          destinationLng
        ...
        mapRef.current?.animateToRegion(
          {
            latitude: riderLat,
            longitude: riderLng,
            latitudeDelta: 0.002, // Adjust to control zoom level
  ...
  const fetchDeliveryDirections = async (
    riderLat,
    riderLng,
    destinationLat,
    destinationLng
    ...
    if (!(await isOnline())) {
      console.warn("No internet connection, skipping fetchDeliveryDirections");
      return;
    }
    try {
      const url = `https://maps.googleapis.com/maps/api/directions/json?origin=${riderLat},${riderLng}&destination=${destinationLat},${destinationLng}&key=AIzaSyAQupqwEHhatglDY4YfsWk-ViolN70UoUg`;
      ...
      if (data.status === "OK" && data.routes[0]) {
        ...
        setCoordinates(decodedCoordinates); // Update the route coordinates
      } else {
  ...
  const fetchStoreDirections = async (
    riderLat,
    riderLng,
    destinationLat,
    destinationLng
    ...
    try {
      ...
      if (data.status === "OK" && data.routes[0]) {
        const encodedPolyline = data.routes[0].overview_polyline.points;
        const decodedCoordinates = decodePolyline(encodedPolyline);
        setCoordinates2(decodedCoordinates); // Update the route coordinates
      } else {
        // console.error("Directions request failed:", data.status);
      }
    } catch (error) {
  ...
  return (
    <SafeAreaView style={{ backgroundColor: "#d1d5db", flex: 1 }}>
      <View>
        <MapView
        ...
        >
          {riderCoords.latitude && (
            <Marker
              ...
              anchor={{ x: 0.5, y: 0.5 }}
              rotation={heading} // Use heading to rotate the marker
              flat={true}
            >
              <Image
                source={{
                  uri: "https://firebasestorage.googleapis.com/v0/b/sprinfare2024.appspot.com/o/images%2Fbike.png?alt=media&token=e2c38b83-b7e3-41a9-b100-ef6ac2da24eb",
                }}
                style={{
                  width: 30,
                  height: 30,
          ...
          {filteredOrders[0]?.orderStatus ===
            ...
            <View>
              {deliveryCoords && (
                <Marker title="Delivery Location" coordinate={deliveryCoords}>
                  <View style={{ width: 20, height: 20 }}>
                    <Image
                      source={{
                        uri: "https://firebasestorage.googleapis.com/v0/b/sprinfare2024.appspot.com/o/images%2Fhome.png?alt=media&token=4433c0e7-7425-4d17-9178-bee60966d007",
                      }}
          ...
          {filteredOrders[0]?.orderStatus ===
            ("Ready" || "Rider Assigned" || "Rider Accepted Order") && (
            <View>
              {storeCoords && (
                <Marker title="Store" coordinate={storeCoords}>
                  <View style={{ width: 20, height: 20 }}>
                    <Image
                      source={{
                        uri: "https://firebasestorage.googleapis.com/v0/b/sprinfare2024.appspot.com/o/images%2Frestaurant.png?alt=media&token=76d07bd6-b763-4ba3-be9f-6acda4b0c7d7",
                      }}
                      style={{ width: 20, height: 20 }}
                      resizeMode="contain"
                    />
      ...
      <BottomSheet
        ref={bottomSheetRef}
        onAnimate={handleSheetChanges} // Track sheet changes
        snapPoints={snapPoints} // Define your snap points
      >
